...
class Grafo():
    ...
    def barrido_pais(self):
        lista_paises_tipo_arq=[]
        lista_paises_tipo_nat=[]
        
        
        for value in self.__elements:
            if value[0].tipo == 'Arquitectónica':
                lista_paises_tipo_arq.append(value[0].pais)
            
        for value in self.__elements:
            if value[0].tipo == 'Natural':
                lista_paises_tipo_nat.append(value[0].pais)
        
        # print(lista_paises_tipo_arq)           
        # print(lista_paises_tipo_nat) 
        
        for j in lista_paises_tipo_arq:
            for k in lista_paises_tipo_nat:
                if j==k:
                   print(f'El pais {j} tiene maravillas Arquitectonicas y Naturales')   
            
        #     if control1==True and control2==True:
        #         print(f'El pais {j} tiene maravillas Arquitectonicas y Naturales')
    
    def barrido_pais_tipo(self):
        lista_paises_tipo_arq=[]
        lista_paises_tipo_nat=[]
        # lista_paises_unica=[]
        # lista_maravilla_tipo=[]
        paises_repetidos_tipo_arq=[]
        paises_repetidos_tipo_nat=[]
        ...
        for i in range(len(lista_paises_tipo_arq)):
            for j in range(len(lista_paises_tipo_arq)):
                if i != j:
                    if lista_paises_tipo_arq[i] == lista_paises_tipo_arq[j] and lista_paises_tipo_arq[i] not in paises_repetidos_tipo_arq:
                        paises_repetidos_tipo_arq.append(lista_paises_tipo_arq[i])
        
        for i in range(len(lista_paises_tipo_nat)):
            for j in range(len(lista_paises_tipo_nat)):
                if i != j:
                    if lista_paises_tipo_nat[i] == lista_paises_tipo_nat[j] and lista_paises_tipo_nat[i] not in paises_repetidos_tipo_nat:
                        paises_repetidos_tipo_nat.append(lista_paises_tipo_nat[i])
        
        
        
        for j in paises_repetidos_tipo_arq:
            print(f'El pais {j} tiene maravillas del mismo tipo' )
        for k in paises_repetidos_tipo_nat:
            print(f'El pais {k} tiene maravillas del mismo tipo' )
                    
        # print(paises_repetidos_tipo_arq)           
        # print(paises_repetidos_tipo_nat) 
